// 公司 ID -- 企业族谱与关系发现
const companyId = getQueryVariable('cid');

if (companyId) {
    const RELATIONS_API = api('relations', { companyId: companyId });
    d3.json(RELATIONS_API, (error, resp) => {
        if (error) {
            console.error('关系图数据请求错误！');
            return;
        }
        if (typeof resp === 'string') {
            try {
                ...
                console.error('无法解析 JOSN 格式错误！', RELATIONS_API);
                return;
            }
        }
        renderForce(resp, companyId);
    });
}

const width = companyId ? 1162 : Math.max(window.innerWidth, 1136);
...
function genDrawinData(resp) {
    ...
    // 将画图数据保存到全局变量
    drawinData = { relations, nodes, links, nodesMap, linkMap, hnum, cnum, idsFrom, idsTo};
}
...
function initialize(ids = '') {
    ...
    // 聚焦光环
    focusHalo = nodeCircle.filter(node => node.ntype === 'Company' && (ids + '').includes(node.id));
...
function tick() {
    // 节点位置
    nodeCircle && nodeCircle.attr('transform', node => 'translate(' + node.x + ',' + node.y + ')');
    // 连线路径
    linkLine && linkLine.attr('d', link => genLinkPath(link));
    // 连线文字位置
    lineText && lineText.attr('dx', link => getLineTextDx(link));
    // 连线文字角度 
    lineText && lineText.attr('transform', function (link) {
        return getLineTextAngle(link, this.getBBox());
    });
}

function genNodes(nodes, coord) {
    for (const node of Object.values(nodes)) {
        Object.assign(node, coord[node.id]);
    }
    return nodes;
}
...
function handleWheelMenu(d) {
    if (d.data.action === 'info') {
        ...
        const url = api('companyInfo', { companyId: hoverCompanyId });
        d3.json(url, (error, resp) => {
            if (error) {
                toggleMask(false);
    ...
    location.href = '../1.html?cid=' + hoverCompanyId + '#' + d.data.hash + ((new Date().getTime() + '').substr(-5));
}
    ...
    if (flag && data) {
        ...
        const html = `
            <div class="company-title">
                ...
                <span class="company-reg-status">${regStatus.replace(/（.*）/g, '')}</span>
            </div>
    ...
    relations.map(function (item) {
        ...
        for (const node of nodes) {
            if (node.id === startNode || node.id === endNode) {
                ...
                node.linkWeight ? node.linkWeight += 1 : node.linkWeight = 1;
            }
        }
    });
...
// 更新画图数据
function updateDrawinData(nodes, relations) {
    // 生成 nodes map
    const [nodesMap, { Human: hnum = 0, Company: cnum = 0 }] = genNodesMap(nodes);
    // 起点和终点相同的关系映射
    const [linkMap] = genLinkMap(relations);
